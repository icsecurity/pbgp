...
/* Message for the secure update for a single nlri */
struct nlri_sec_upd {
	struct aspath *asp;
	time_t *tsarray;
	nlri_t nlri;
...
typedef struct nlri_sec_upd nlri_sec_upd_t;
...
int
pi_verify_update(void*, pi_rcvup_t*);

int
pi_rcvup_table_init(pi_rcvup_t** htab, u_int16_t number,secext_t* pi_setup);

int
pi_rcvup_table_clear(pi_rcvup_t** pi_table);

int
pi_rcvup_add(pi_rcvup_t **htab, struct rcvent_key *key, const uint8_t* data,uint32_t size);
