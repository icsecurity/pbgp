...
pi_init(char*pubca, char*parca, char*asenv, secext_t **ext)
{
    ...
    *ext            = pext;
	...
	rv = rbnlri_init();
	if(rv != 0) {
		log_warnx("pi_verify_as_open:: initialization for the prefix tree failed");
		goto err4;
	}
	pext->head = &se_table;

	/* Add entries for the owned prefixes */
	for(i =0; i< pext->data->pfixlist->size; i++) {
		nlri_sec_upd_t tmp;
		bzero(tmp,sizeof(nlri_sec_upd_t));

		tmp->asp = NULL;
		tmp->tsarray = &pext->data->pfixlist->tsca[i];
		tmp->nlri.pfixlen = pext->data->pfixlist->netmask[i];
		tmp->nlri.prefix = pext->data->pfixlist->ina[i];
		tmp->signature = pext->data->pfixlist->pf_sign[i];

		rv = rbnlri_addelem(pext->head ,&tmp);
		if(rv != 0) {
			log_warnx("pi_verify_as_open:: rbnlri_addelem");
			goto err5;
		}
	}
    return 0;
err5:
	rbnlri_clear(pext->head);
err4:
	acc_clear(acc);
err3:
	join_clear(env);
...
pi_sign_update(uint8_t **vrfupd, void *update, size_t datalen,struct peer *s_peer ,secext_t* pi_setup)
{
	...
	if (attrpath_len != 0) { /* 0 = no NLRI information in this message */
		...
		while(tlen > 0 ) {
			pos += rde_update_get_prefix(p + pos, nlri_len, prefix,&pfixlen);
			pfix_count++;
		}

		//size of ts list
		vrfupd_size += sizeof(time_t)*(asp->aspath->ascnt +1);
		//size of signature size var and of the signature itself
		vrfupd_size += sizeof(uint16_t);
		sign_size =  get_ibesignature_size(pi_setup->setup,asp->aspath->ascnt);
		vrfupd_size += sign_size;

		vrfupd_size = pfix_count*vrfupd_size;
		//add the number of blocks var
		vrfupd_size += sizeof(uint16_t);

		*vrfupd = malloc(vrfupd_size );
		if(!(*vrfupd)) {
			log_warn("pi_sign_update() :: malloc() :: %s",strerror(errno));
			goto err;
		}
		bzero(*vrfupd,vrfupd_size);
		vrfp = *vrfupd;

		WR_ELEM(vrfp,pfix_count,2);

		pos = 0;
		/* Forge a signature for each NLRI into the update*/
		while(len > 0) {
			/* If len_aspath > 1, retrieve the aggregated signature in the previous step */

			struct bgpd_addr *prefix = NULL;
			struct tsl_entry ts;
			nlri_t nlri;
			uint8_t pfixlen =0;
			pi_rbnlri_t *node = NULL;
			uint32_t firstas = 0;
			u_char *pas= NULL;
			time_t *tsa = NULL;

			/* For each NLRI */
			nlri_sec_upd_init(&sup);
			bzero(&ts,sizeof(struct tsl_entry));

			pos = rde_update_get_prefix(p, nlri_len, prefix,&pfixlen);

			nlri.prefix = prefix.ba.v4 ;
			nlri.pfixlen = pfixlen;

			//don't consider the first entry (the signing node) of the aspath during lookup
			asp->aspath->ascnt--;
			memcpy(&firstas,asp->aspath->data + 2,4); //backup
			memcpy(asp->aspath->data + 4,asp->aspath->data,2);
			pas = asp->aspath->data;
			asp->aspath->data+=4;

			rv = rbnlri_searchelem(&node,pi_setup->head,&nlri,asp->aspath);
			//this case should neve happens
			if(rv <= 0) {
				log_warnx("rbnlri_addelem :: %s",strerror(errno));
				goto err;
			}
			//this prefix has been never announced before
			if(node->announced_ts == 0) {
				node->announced_ts = time(NULL);
			}
			WR_ELEM(vrfp,node->announced_ts,4);

			rbnlri_gettspath(&tsa, pi_setup->head, &nlri, asp->aspath);

			for(ii = 0; ii < asp->aspath->ascnt; ii++)
				WR_ELEM(vrfp,tsa[ii],4);

			WR_ELEM(vrfp,sign_size,2);

			//restore the complete aspath for signing
			asp->aspath->ascnt++;
			memcpy(pas+2,&firstas,4);
			asp->aspath->data = pas;

			nlri_len -= pos;
			p += pos;

			/* Generate the new message to sign (AS0,...,ASY,TS,NLRI)*/
			/* Aggregate the signature */

			/* Append data to output */
			nlri_sec_upd_clear(upd);
		}
    ...
    /* Print ASPAth */
    /*log_warnx("----- START OF ASPATH -----\n");

    asp_curs = asp->aspath->data +2;
    for (i=0; i< asp->aspath->ascnt; i++)
    {
        uint32_t as_tmp = htonl(*((uint32_t*)( asp_curs )));
        log_warnx("AS%d ", as_tmp);

        asp_curs += 4 ;
    }
    log_warnx("----- END OF ASPATH -----\n");
*/
    //XXX: a more accurate cleanup is required
    free(asp);
    asp = NULL;
...
int
nlri_sec_upd_init(nlri_sec_upd_t **upd)
{
	nlri_sec_upd_t *p = NULL;


	p = (nlri_sec_upd_t *) calloc(sizeof(nlri_sec_upd_t));
	if(!p) {
		log_warnx("nlri_sec_upd_init :: calloc (%s)",strerror(errno));
		return -1;
	}
...
void
nlri_sec_upd_clear(nlri_sec_upd_t *upd)
{
	if(p) {
		while (p->tslist != NULL)           /* Delete. */
			LIST_REMOVE(p->tslist, entry);
